...
class single_choice_input():
    def __init__(self, bg_color, label_color, Label_Font, frame, StringVarList, index_dict, WIDTH, Rows, Columns, Header, header_index, columnwidth):
        ...
        #subframe
        mini_frame = Frame(frame, bg=bg_color)
        mini_frame.place(relx=0, rely=0.2, relwidth=1, relheight=.5)
...
class zuordnungsfrage_input():
    def __init__(self, bg_color, label_color, Label_Font, frame, StringVarList, index_dict, WIDTH, Rows, Columns, Header, header_index, columnwidth):
        ...
        for row in range(self.Rows):
            index = index_dict["definitions_response_{}_text" .format(row+1)]
            #self.varlist = [] todo wozu ist das da?
            rowlist.append(choice_row(self.StringVarList, second_frame, row, relwidth , index, self.Columns, self.columnwidth))
...
class zuordnungspaare():
    def __init__(self, bg_color, label_color, Label_Font, frame, StringVarList, index_dict, WIDTH, Rows, Columns, Header, header_index, columnwidth):
        ...
        for row in range(self.Rows):
            index = index_dict["assignment_pairs_definition_{}" .format(row+1)]
            #self.varlist = [] todo wozu ist das da?
            rowlist.append(zuordnungs_row(self.StringVarList, second_frame, row, relwidth, index, self.Columns, self.columnwidth))
...
class zuordnungs_row():
    def __init__(self, Var_list, second_frame, row, relwidth, index, columns, columnwidth):
        self.columns = columns
        self.rowlist = []
        self.Var_list = Var_list
        for column in range(self.columns):
            self.rowlist.append(zuordnungs_column(second_frame, row, column, int(relwidth * columnwidth[column]), Var_list[index + column][0]))
            if column == 0:
                #self.rowlist[column].enable()
                self.textvar_index = index + column
        #self.binder()
